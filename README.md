...
|---|---|
| **Deployment freeze windows** | A service that blocks workflow execution based on a calendar, with org-wide scope and RBAC. GitHub environment protection rules are per-environment only, with no calendar primitive. |
| **ITSM / change management integration** | A custom GitHub App that creates ServiceNow/Jira change tickets pre-deployment, polls for approval state, gates execution on approval, and closes/updates tickets on completion. GitHub has no native ITSM integration; this requires building and maintaining the App + REST API layer. |
| **DORA metrics** | A data pipeline collecting deployment frequency, lead time, change failure rate, and MTTR from workflow events, cross-referenced with incident data. GitHub's Q1 2026 public roadmap contains no DORA capability. The `governance/metrics-collector/dora-metrics-design.md` file in this repo is a design document — not running code. |
| **Artifact promotion model** | A registry of artifacts mapped to environments, with promotion as an explicit platform action (not re-tagging). What exists here: SHA-tagged images deployed directly. |
| **Database deployments** | Flyway or Liquibase wired into the pipeline with migration state tracking, approval gates for schema changes, and a rollback strategy for failed migrations. Nothing in this repo. |
| **Progressive delivery for non-K8s targets** | The `gitops/apps/prod/user-service/rollout.yaml` defines an Argo Rollout for K8s. Canary/blue-green for Lambda, ECS, and VMs requires separate implementations for each target type. |
| **Deployment domain model** | Services, environments, and infrastructure definitions as reusable platform entities. What GitHub provides: environment labels with shared protection rules. The mapping between a service, its infrastructure, and its deployment configuration lives in each repo's YAML. |
...
For the business case on what that build costs and how it compares to a purpose-built CD platform, see the [Build vs Buy comparison](./Harness%20CD%20vs%20GitHub%20Actions%20%20Build%20vs%20Buy%20V1.xlsx).
